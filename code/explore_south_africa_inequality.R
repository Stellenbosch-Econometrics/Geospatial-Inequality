...
# Cape town 
ind_ct = st_contains(SA_ineq_10km_hex_all$geom, st_centroid(subset(H7_grid_all, CAT_B == "CPT", geom))) %>% 
  lengths() %>% is_greater_than(0) %>% which()

# Matching with Uber Hexagons
ind = st_contains(SA_ineq_10km_hex_all$geom, st_centroid(H7_grid_all$geom)) 
...
SA_ineq_10km_hex_all[tmp$HEX96_row, names(tmp)[-1]] <- tmp[, -1]


ind_all = ind %>% lengths() %>% is_greater_than(0) %>% which()

SA_ineq_10km_hex_all <- SA_ineq_10km_hex_all[ind_all, ]

plot(SA_ineq_10km_hex_all[ind_ct, .c(gini_2020, IWI_w_GINI, RWI_w_GINI, NL21_w_GINI)] %>% 
...
SA_ineq_10km_hex_all %>% qDT() %>% gvr("w_GINI|gini_2020|WGINI") %>%
