...
NL20[NL20 < -1] <- NA_real_
NL20 %<>% terra::aggregate(fact = 2, fun = "mean", na.rm = TRUE) %>% 
     as.data.frame(xy = TRUE) %>% set_names(.c(lon, lat, NL20)) %>% 
     fsubset(NL20 >= 0 & between(lon, minlon, maxlon) & between(lat, minlat, maxlat))
...
NL20 %$% round_to_kms_fast(lon, lat, 0.7) %>% GRPN(FALSE) %>% descr()
...
NL20_round <- NL20 %>% ftransform(round_to_kms_exact(lon, lat, 0.7)) %>% 
  fgroup_by(lon, lat, sort = FALSE) %>% fselect(NL20) %>% fmean() %>% 
  merge(WPOP %>% ftransform(round_to_kms_exact(lon, lat, 0.7)) %>% 
          fgroup_by(lon, lat, sort = FALSE) %>% fsum(), by = .c(lon, lat))

# Removing original files and freeing memory (garbage collection)
rm(NL20, WPOP, IWI, RWI); gc()
...
settransform(NL20_round, cell = dgGEO_to_SEQNUM(world_96km2_hex, lon, lat)$seqnum)
...
NL20_ineq_96km2_hex <- NL20_round %>% 
  fsubset(GRPN(cell) > 5L & fsd(NL20, cell, TRA = 1) > 0) %>% 
  fgroup_by(cell) %>% 
  fsummarise(N = GRPN(),
             pop = fsum(pop),
             mean = fmean(NL20),
             w_mean = fmean(NL20, pop),
             median = fmedian(NL20),
             w_median = fmedian(NL20, pop, ties = "q7"), 
             GINI = gini_wiki(NL20), # Gini Index: Area above Lorentz Curve
             w_GINI = w_gini(NL20, pop),
             TI = fmean(fmean(NL20+1, TRA = "/") %>% multiply_by(log(.))), # Theil Index (Overall)                                                    
             HI = fsum(fsum(NL20, TRA = "/")^2)) %>%  # Herfindahl Index
  fmutate(NHI = (HI - 1/N)/(1-1/N)) # Normalized Herfindahl Index
...
ineq_96km2_hex_all <- IWI_ineq_96km2_hex %>% add_stub("IWI_", cols = -1L) %>% 
  ...
  merge(NL20_ineq_96km2_hex %>% add_stub("NL20_", cols = -1L), by = "cell", all = TRUE) %>% 
  fsubset(is.finite(IWI_mean) | is.finite(RWI_mean) | is.finite(NL20_mean))
...
settransform(ineq_96km2_hex_all, 
   GINI_mean = psum(list(IWI_GINI, RWI_GINI, NL20_GINI) %>% TRA(proportions(1/fmean(.)), "*")),
   WGINI_mean = psum(list(IWI_w_GINI, RWI_w_GINI, NL20_w_GINI) %>% TRA(proportions(1/fmean(.)), "*")),
   TI_mean = psum(list(IWI_TI, RWI_TI, NL20_TI) %>% TRA(proportions(1/fmean(.)), "*")),
   HI_mean = psum(list(IWI_HI, RWI_HI, NL20_HI) %>% TRA(proportions(1/fmean(.)), "*")),
   NHI_mean = psum(list(IWI_NHI, RWI_NHI, NL20_NHI) %>% TRA(proportions(1/fmean(.)), "*"))
)
...
# I use Nightlights layer to get the appropriate cells (the layer was masked in GEE using a high-res country boundary, setting all values outside the mask to -9999)
NL20 <- terra::rast("data/south_africa_viirs_dnb_nightlights_v1_vcmslcfg_annual_median_composite/2020.tif") 
...
system.time({
  NL20_1km_cells = NL20_round %$% upsample_cells(world_1km_hex, lon, lat, 0.3, 10) %>% intersect(NL_1km_cells$cell)
})

NL20_1km_cells <- dgSEQNUM_to_GEO(world_1km_hex, NL20_1km_cells) %>% rm_stub("_deg", pre = FALSE) %>% 
                  add_vars(cell = NL20_1km_cells, pos = "front") %>% qDT()

# Creating S2 geometries
NL20_s2 = NL20_round %$% s2_lnglat(lon, lat)
NL20_1km_cells_s2 = NL20_1km_cells %$% s2_lnglat(lon, lat)

# This takes quite long
s2_distance(NL20_s2, NL20_1km_cells_s2[[1]])

degree_grid = NL20_round %$% expand.grid(lon = seq(floor(min(lon)), ceiling(max(lon)), 0.25),
                                         lat = seq(floor(min(lat)), ceiling(max(lat)), 0.25))

# These estimates can take a few hours to complete, even on a fast machine... 
NL20_1km_cells$GINI_5km = gridded_distance_interpol(degree_grid, NL20_round$NL20, NL20_s2, NL20_1km_cells_s2, 
                                               thresh = 5000, step = 0.25, tol = 0.05, FUN = w_gini, verbose = TRUE)

NL20_1km_cells$WGINI_5km = gridded_distance_interpol(degree_grid, NL20_round$NL20, NL20_s2, NL20_1km_cells_s2, 
                                               thresh = 5000, step = 0.25, tol = 0.05, FUN = w_gini, w = NL20_round$pop, verbose = TRUE)

NL20_1km_cells$GINI_10km = gridded_distance_interpol(degree_grid, NL20_round$NL20, NL20_s2, NL20_1km_cells_s2, 
                                               thresh = 10000, step = 0.25, tol = 0.1, FUN = w_gini, verbose = TRUE)

NL20_1km_cells$WGINI_10km = gridded_distance_interpol(degree_grid, NL20_round$NL20, NL20_s2, NL20_1km_cells_s2, 
                                               thresh = 10000, step = 0.25, tol = 0.1, FUN = w_gini, w = NL20_round$pop, verbose = TRUE)

NL20_1km_cells %<>% replace_Inf() %>% na_omit(cols = names(.) %like% "GINI", prop = 0.5)
